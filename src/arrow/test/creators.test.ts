...
import { Arrow, resolve } from "../index";
import { Left, Right } from "../../either";

it("Arrow should Arrow", async () => {
  const result = await Arrow<{}, never, number>(async () =>
    Right(1)
  ).runAsPromiseResult({});
...
it("Arrow should drawAsync", async () => {
  const result = await drawAsync<number>(async () => 1).runAsPromiseResult({});
  expect(result).toEqual(1);
});

it("Arrow should drawFunction", async () => {
  const result = await drawFunction<number>(() => 1).runAsPromiseResult({});
  expect(result).toEqual(1);
});

it("Arrow should drawEither - right", async () => {
  const result = await drawEither<{ ok: number }, never, number>(({ ok }) =>
    Right(1)
...
it("Arrow should drawEither - left", async () => {
  const { error } = await drawEither<{ ok: number }, number, never>(({ ok }) =>
    Left(1)
  ).runAsPromise({ ok: 1 });
...
it("Arrow should drawFailableAsync - success", async () => {
  const result = await drawFailableAsync<number>(
    async () => 1
  ).runAsPromiseResult({});
  expect(result).toEqual(1);
});
...
it("Arrow should drawFailableFunction - success", async () => {
  const result = await drawFailableFunction<number>(() => 1).runAsPromiseResult(
    {}
  );
  expect(result).toEqual(1);
});
...
it("Arrow should drawFailableFunction - failure", async () => {
  const { result, error } = await drawFailableFunction<number>(() => {
...
it("Arrow should drawNullable - null", async () => {
  const { result, error } = await drawNullable<{ ok?: number }, number>(
    ({ ok }) => ok
  )
    .map((a: number) => a + 1)
    .leftMap((a) => "its null")
...
it("Arrow should draw from dependencies", async () => {
  ...
  const sendViaClient = (name: string) =>
    draw((client: Client) => client.send(name));
  const result = await sendViaClient("jj").runAsPromiseResult(client);
...
it("convertAsync should convert async", async () => {
  ...
  const result = await myFuncArrow("ok").runAsPromiseResult({});
...
it("convertAsync should convert async", async () => {
  ...
  const result = await myFuncArrow("ok").runAsPromise({});
