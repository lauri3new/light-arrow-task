...
import { all, Arrow, race } from "../index";
...
it("Arrow should ifOrElse", async () => {
  ...
  const { result, error } = await ifOrElse((a) => a === 1, a, b).runAsPromise(
    {}
  );
...
it("Arrow should ifOrElse", async () => {
  ...
  const { result, error } = await ifOrElse((a) => a === 2, a, b).runAsPromise(
    {}
  );
...
it("Arrow should andThen", async () => {
  const a = await Arrow<{ num: number }, never, { num: number }>(
    async ({ num }) => Right({ num: num + 1 })
  );

  const result = await andThen(a, a, a).runAsPromiseResult({ num: 2 });
