...
it("Arrow should flatMapIf - true", async () => {
  ...
  const result = await Arrow<{}, never, number>(async () => Right(1))
    .flatMapIf(condition, (a) =>
      Arrow<{}, never, number>(async () => Right(a * 3))
    )
    .runAsPromiseResult({});
  expect(result).toEqual(3);
});
...
it("Arrow should leftMap", async () => {
  const { error } = await Arrow<{}, number, never>(async () => Left(1))
    .leftMap((a) => a * 3)
    .runAsPromise({});
...
it("Arrow should leftFlatMap - right", async () => {
  const { error } = await Arrow<{}, never, number>(async () => Right(1))
    .leftFlatMap((a) => resolve(a * 3))
    .runAsPromise({});
...
it("Arrow should orElse", async () => {
  const a = Arrow<{}, number, never>(async () => Left(1)).orElse(
    Arrow<{}, number, never>(async () => Left(2))
  );
...
it("Arrow should ifOrElse", async () => {
  const a = Arrow<{}, string, never>(async () => Left("hey")).ifOrElse(
    (c) => c === "wasup",
    Arrow<{}, string, never>(async () => Left("yo"))
  );
...
it("Arrow should bracket", async () => {
  let flag = false;
  const a = Arrow<{}, never, { ok: number }>(async () =>
    Right({ ok: 123 })
  ).bracket((b) => {
  ...
  const { result, context } = await a.runAsPromise({});
...
it("Arrow should bracket - fail case", async () => {
  ...
  const { result, error, context } = await a.runAsPromise({});
  expect(flag).toEqual(true);
...
it("Arrow should run - error", async () => {
  const a = Arrow<{ ok: () => number }, number, never>(async (a) =>
    Left(a.ok())
  );
  const result = a.run(
    { ok: () => 2 },
...
it("provide should eliminate dependency", async () => {
  ...
  type twoDeps = depA & depB;
  const a = Arrow<twoDeps, never, number>(async (a) => Right(a.ok()));
  const x = provideSome({ ok: () => 3, nok: () => "yo" } as twoDeps)<
    {},
    never,
    number
  >(a);
...
it("Arrow should preserve basic type dep", async () => {
  const result = await Arrow<string, never, number>(async () => Right(1))
    .map((a) => a * 3)
    .flatMap(() =>
      provideSome("xhe")(Arrow<string, never, string>(async (a) => Right(a)))
    )
    .runAsPromiseResult("hey");
