...
import { Arrow, construct, constructTask, resolve } from "../index";
...
it("constructTask should run - success", async () => {
  const a = construct<{ ok: () => number }, never, number>(
    (a) => (res) => res(a.ok())
  );
...
it("constructTask should run - error", async () => {
  const a = construct<{ ok: () => number }, number, never>(
    (a) => (_, rej) => rej(a.ok())
  );
  const result = a.run(
    { ok: () => 2 },
...
it("constructTask should run - failure", async () => {
  const a = construct<{ ok: () => number }, any, never>((a) => (_, rej) => {
    rej("boom");
  });
  const result = a.run(
    { ok: () => 2 },
